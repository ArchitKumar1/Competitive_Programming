...
int mx=1000000;
vector<ll> primes(mx,0),pr,product;
void sieve(){
	for(ll i=2;i*i<=mx;i++){
		if(primes[i]==0){
			for(ll j=2*i;j<=mx;j=j+i)
			primes[j]=1;
		}
	}
	for(ll i=2;i<=mx;i++){
...
int main()
{
	...
	cin>>n;
	n=n-9;
	ll a=1,b=2,c,d,flag=0;
	for(ll i=2;i<=cbrt(n)+1;i++)
	{
		ll k=i*i*i;
		if(binary_search(product.begin(),product.end(),n-k))
		{
			c=min(cbrt(k+1),cbrt(n-k));
			d=max(cbrt(k+1),cbrt(n-k));
			flag=1;
			break;
		}
	}
