...
void solve(){
   ...
   for(int i = 1;i<=n;i++){
       for(int j = i;j<=n;j++){
           int middle = 0;
           forn(k,26){
               middle = max(middle,pref[j][k] - pref[i-1][k]);
           }
           int lanr = 0;
           forn(k,26){
               int x = pref[i-1][k] - pref[0][k] ;
               int y =  pref[n][k]- pref[j][k];
               if(x > 0 && y > 0){
                   
                   lanr = max(lanr,2*min(x,y));
               }
            //    trace(x,y,lanr);
           }
        // trace(ans,lanr,middle,i,j);
           ans = max(ans,lanr+middle);
       }
   }
   cout << ans << endl;
}
